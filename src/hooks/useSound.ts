...
export function unlockAudio() {
  try {
    ...
    // شغّل نغمة صامتة فعلية — هذا يفتح القناة الصوتية نهائياً على الآيفون
    if (!_audioUnlocked) {
...
export function useSound() {
  ...
  // يشغّل دالة صوت بعد التأكد أن السياق نشط (يحلّ مشكلة التعليق على الآيفون)
  const withCtx = (fn: (ac: AudioContext, t: number) => void) => {
    const ac = ctx();
    if (!ac) return;
    const run = () => fn(ac, ac.currentTime + 0.02);
    if (ac.state === "suspended") {
      ac.resume().then(run).catch(() => { try { run(); } catch {} });
    } else {
      run();
    }
  };
