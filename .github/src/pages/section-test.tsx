          ...
          {ex && (
            <motion.div key={idx} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1">
              <div className="text-center mb-6">
                ...
                {/* التعليمة حسب النوع */}
                <div className="text-sm text-muted-foreground mb-3" style={{ direction: "rtl" }}>
                  {ex.type === "translate" ? "اختر الترجمة الإنجليزية الصحيحة" : "اختر الكلمة المناسبة لإكمال الجملة"}
                </div>
                {/* نص السؤال */}
                {ex.type === "fill_blank" ? (
                  <div className="text-xl font-extrabold" style={{ direction: "ltr" }}>
                    {(ex.blankSentence ?? "").replace("___", " ____ ")}
                  </div>
                ) : (
                  <div className="text-2xl font-extrabold" style={{ direction: "rtl" }}>
                    {ex.arabic}
                  </div>
                )}
              </div>
